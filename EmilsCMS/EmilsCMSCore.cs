...
namespace EmilsCMS.core
{
    internal class EmilsCMSCore
    {
        ...
        const string AppName = "Mongo console app";  // Nom de l'application
        ...
        const string AppDate = "2026-02-05"; // Date de création ou de dernière mise à jour
...
";
        ...
        // Global data
        RepositoryOuvrages ouvrages = new(); // Liste des ouvrages
        MongoDBService? mongoService = null;    // Service MongoDB (nullable)
        // =================================================================
        // HELPER METHODS
        // =================================================================
        ...
        void MainMenu(bool showError = false)
        {
            ...
            {
                LoadSettings();
                LoadUserData();  // Charge toujours depuis MongoDB (ou JSON en fallback)
                needLoad = false;
                Console.WriteLine();
            }
        ...
        void ShowItemsByType<T>() where T : Ouvrage
        {
            ...
            {
                nameof(Livre) => "Livres",
                nameof(BandeDessinee) => "Bandes dessinées",
        ...
        void ShowAllItems(string searchQuery = "")
        {
            ...
            {
                var livres = allOuvrages.OfType<Livre>().Where(l => l is not BandeDessinee).ToList();
                var bds = allOuvrages.OfType<BandeDessinee>().ToList();
                    ...
                    else
                    {
                        ...
                        switch (input)
                        {
                            ...
                            case '3':
                                ...
                                break;
                            case '4':
                                if (currOuvrage is BandeDessinee bd4)
                                ...
                                break;
                            case '5':
                                if (currOuvrage is BandeDessinee bd5)
                                ...
                                break;
                            case '6':
                                if (currOuvrage is BandeDessinee bd6)
                                {
        ...
        void TestMongoConnection()
        {
            ...
            if (settings.MongoDbPassword == null)
            {
                Console.WriteLine("[ERREUR] Configurez d'abord le mot de passe");
                Console.WriteLine();
                Console.WriteLine("Appuyez sur Entrée...");
                Console.ReadLine();
                SettingsMenu();
            ...
            try
            {
                ...
                var testService = new MongoDBService(settings.MongoDbPassword);
                
                if (testService.TestConnection())
                    Console.WriteLine("[OK] Connecté ✓");
                else
                    Console.WriteLine("[ERREUR] Échec ✗");
            }
            catch (Exception ex)
            {
        ...
        void LoadUserData()
        {
            ...
            if (settings.MongoDbPassword == null)
            {
                ...
                Console.WriteLine("[INFO] Chargement depuis JSON local...");
                LoadFromJson();
                return;
            }
            ...
            try
            {
                mongoService = new MongoDBService(settings.MongoDbPassword);
                ouvrages = LoadUserDataAsync().GetAwaiter().GetResult();
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine("[INFO] Basculement vers JSON local...");
                LoadFromJson();
            }
        }

        async Task<RepositoryOuvrages> LoadUserDataAsync()
        {
            return await mongoService!.LoadAllAsync();
        }

        /// <summary>
        /// Sauvegarde dans MongoDB + JSON après chaque modification
        /// </summary>
        void SaveUserData()
        {
            if (settings.MongoDbPassword == null || mongoService == null)
            {
                Console.WriteLine("[WARN] MongoDB non disponible, sauvegarde JSON uniquement");
                SaveToJson();
                return;
            }
            ...
            try
            {
                ...
                SaveUserDataAsync().GetAwaiter().GetResult();
                Console.WriteLine($"[OK] Sauvegardé dans MongoDB");
            ...
            {
                Console.WriteLine($"[ERREUR] MongoDB : {ex.Message}");
                Console.WriteLine("[WARN] Sauvegarde JSON uniquement");
                SaveToJson();
            }
        }
        ...
        {
            await mongoService!.SaveAllAsync(ouvrages);
        }

        /// <summary>
        /// Charge depuis JSON (fallback)
        /// </summary>
        void LoadFromJson()
        {
            try
            {
                if (File.Exists("data.json"))
                {
                    ...
                    ouvrages = JsonConvert.DeserializeObject<RepositoryOuvrages>(json, jsonSettings) ?? new();
                    Console.WriteLine($"[OK] {ouvrages.Ouvrages.Count} ouvrages chargés (JSON)");
                }
                else
                {
                    Console.WriteLine("[INFO] Démarrage avec liste vide");
                }
            }
            catch (Exception ex)
            {
        ...
        void SaveToJson()
        {
            try
            {
                ...
                string json = JsonConvert.SerializeObject(ouvrages, jsonSettings);
                File.WriteAllText("data.json", json);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[WARN] Backup JSON échoué : {ex.Message}");
            }
        }
